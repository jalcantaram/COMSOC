...
@section('container')  
  <div class="divGeneral">
      ...
      <br>
      <div class="row">
        <form class="form-horizontal" action="{{ url('/exportcsvRech') }}" enctype="multipart/form-data">
          ...
          @foreach($fichas as $ficha)
            <tr>
              ...
              @if($ficha['detalleComision']['nombre'] == 'nacional')
                @if(isset($ficha['registroPersonalNacional']['comisionados'][0]['saliendoMunicipioNacional']))
                  {{ $ficha['registroPersonalNacional']['comisionados'][0]['saliendoMunicipioNacional'] }}
                @endif
              @else
                @if(isset($ficha['registroPersonalInternacional']['comisionados'][0]['saliendoEntidadInternacional']))
                  {{ $ficha['registroPersonalInternacional']['comisionados'][0]['saliendoEntidadInternacional'] }}
                @endif
              @endif
              <strong>-</strong>
              @if($ficha['detalleComision']['nombre'] == 'nacional')
                @if(isset($ficha['registroPersonalNacional']['comisionados'][0]['llegandoMunicipioNacional']))
                  {{ $ficha['registroPersonalNacional']['comisionados'][0]['llegandoMunicipioNacional'] }}
                @endif
              @else
                @if(isset($ficha['registroPersonalInternacional']['comisionados'][0]['llegandoEntidadInternacional']))
                  {{ $ficha['registroPersonalInternacional']['comisionados'][0]['llegandoEntidadInternacional'] }}
                @endif
              @endif
              </td>
              <td align="center">
              ...
              @elseif(Session::get('user.roles.1') == 'Viatinet_Dga' || Session::get('user.roles.1') =='Viatinet_supDga')
                <td align="center">
                  <a href="{{ url('/validaDga?sess='.Session::get('sessionId').'&f='.$ficha['_id']) }}" class="btn btn-default" role="button">
                    <span class="glyphicon glyphicon-saved" aria-hidden="true"></span>
                  </a>
                </td>
...
<script type="text/javascript">
  ...
  var countRech = "{{ count($fichas) }}", countAut = "{{ $fichasAut }}";
  if (countAut != 0) {
