...
<div class="divGeneral">
  <div class="forminfo">
    ...
    @endif 
    <br>
    @if(($datos['data']['status'] != 'rechazadoDGA' && $datos['data']['status'] != 'Autorizado') || ($datos['data']['status'] == 'firmaDGA'))
      ...
      <div class="panel panel-default">
        ...
        <div id="collapseOne" class="panel-collapse collapse in">
          <div class="panel-body">
            ...
            <br>
            @if($datos['data']['status'] == 'firmaDGA' || $datos['data']['status'] == 'rechazadoDGA')
...
<div class="modal fade" id="myModalRechazar" tabindex="-1" role="dialog" aria-labelledby="gridModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form id="rechazaForm" action="{{ url('/regObsDga') }}" enctype="multipart/form-data" accept-charset="UTF-8" method="post">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
...
<div id="myModalFirma" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="gridModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div style="width: 100%; text-align: center;" id="modalFooter"></div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    $(".muestraModalFirma2").click(function(e){
      ...
      if(validacion == 0){
        validacion = 1;
        var idViatico = "{{ $datos['data']['_id'] }}";
        var nombreViatico = "{{ $datos['data']['detalleComision']['nombre'] }}";
        var tipo = "{{ $datos['data']['status'] }}";
        $.ajax({
          url:'{{ url("/pdfFirmaDga")}}',
          data: '_token={{ csrf_token() }}&docId='+idViatico,
          type: 'post',
          dataType: 'json',
          beforeSend: function(){
          ...
          success: function(response){
            ...
            '<input type="hidden" name="_token" value="{{ csrf_token() }}">'+
            '<input type="hidden" name="docId" value="'+idViatico+'">'+
            '<input type="hidden" name="type" value="'+tipo+'">'+
            '</form>'+
            '<form id="firmaDga">'+
            '<input type="hidden" name="_token" value="{{ csrf_token() }}">'+
            '<input type="hidden" value='+idViatico+' name="docId">'+
            '<input type="hidden" value='+nombreViatico+' name="nombre">'+
            '</form>'+
            '<a class="btn btn-success" id="download"><i class="fa fa-download" aria-hidden="true"></i></a>&nbsp;&nbsp' + 
            ...
            '<button class="btn btn_cdmx" id="firmarDga">Firmar documento</button>';
            ...
            $("#firmarDga").click(function(e){
              e.preventDefault();
              if(firma == 0){
                firma = 1;
                $.ajax({
                  url:    '{{ url("/signDocDga") }}',
                  data:   $("form#firmaDga").serialize(),
                  type:   'post',
                  dataType:   'json',
                  success:  function(response){
                    if(response.error.code == 0){
                      ...
                      setTimeout("location.href='"+'{{ url("/autorizadoDga")}}'+"'",3000);
                    }else{
                      firma = 0;
                      $('#mensajeModal .modal-body').css('background-color',response.color);
                      $('#mensajeModal .modal-body').text(response.error.msg);
  ...
  $('.myModalRechazar').click(function(e){ $('#myModalRechazar').modal('show'); });
  $('.ceo').click(function(){
